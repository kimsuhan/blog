...
- PostgreSQL full-text search를 1차 검색 방식으로 사용한다.
- `data/search-index.json`은 생성하지 않고, 필요 시 클라이언트 캐시 용도로만 후속 검토한다.
- 게시글 생성은 draft 저장이므로 검색 인덱스를 만들지 않는다.
- 게시글 수정/발행 시 published 상태만 `post_search_index`에 upsert한다.
- 게시글 보관 시 `post_search_index`에서 제거한다.
- `/search?q=`는 DB 검색 인덱스를 서버 렌더링으로 조회한다.
